...
type Node = { id: number; x: number; y: number; type: "source" | "target" | "junction"; };
type Connection = { from: number; to: number; };

export function PuzzleMinigame({ onComplete }: Props) {
  ...
  useEffect(() => {
    if (connections.length >= 2 && started && !finished) {
      const hasSource = connections.some(c => c.from === 0 || c.to === 0);
      const hasTarget = connections.some(c => c.from === 3 || c.to === 3);
      if (hasSource && hasTarget) {
        setCorrectPuzzles(prev => prev + 1);
        if (puzzleIndex < 2) {
          setTimeout(() => { setPuzzleIndex(prev => prev + 1); generatePuzzle(); }, 500);
        } else {
  ...
  const handleNodeClick = useCallback((nodeId: number) => {
    ...
    else if (selectedNode === nodeId) setSelectedNode(null);
    else {
      const exists = connections.some(c => (c.from === selectedNode && c.to === nodeId) || (c.from === nodeId && c.to === selectedNode));
      if (!exists) setConnections(prev => [...prev, { from: selectedNode, to: nodeId }]);
      setSelectedNode(null);
    }
    ...
    return (
      <Card className="p-8 text-center space-y-6" data-testid="card-minigame-intro">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-foreground">Circuit Connect</h2>
    ...
    return (
      <Card className="p-8 text-center space-y-6 pop-in" data-testid="card-minigame-result">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">{correctPuzzles} / 3 circuits connected</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card className="p-6 space-y-4" data-testid="card-minigame-active">
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground">Circuit {puzzleIndex + 1} of 3</div>
        <div className={cn("text-2xl font-bold", timeLeft <= 15 ? "text-destructive" : "text-foreground")}>{timeLeft}s</div>
      </div>
      <div className="relative w-full h-64 bg-muted/50 rounded-lg">
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          {connections.map((conn, i) => {
            const fromNode = nodes.find(n => n.id === conn.from);
            const toNode = nodes.find(n => n.id === conn.to);
            if (!fromNode || !toNode) return null;
            return <line key={i} x1={`${fromNode.x}%`} y1={`${fromNode.y}%`} x2={`${toNode.x}%`} y2={`${toNode.y}%`} stroke="hsl(var(--primary))" strokeWidth="3" strokeLinecap="round" />;
          })}
        </svg>
        {nodes.map(node => (
          <button key={node.id} onClick={() => handleNodeClick(node.id)} className={cn("absolute -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full flex items-center justify-center transition-all", node.type === "source" && "bg-yellow-400 border-2 border-yellow-600", node.type === "target" && "bg-blue-400 border-2 border-blue-600", node.type === "junction" && "bg-gray-400 border-2 border-gray-600", selectedNode === node.id && "ring-4 ring-primary scale-110")} style={{ left: `${node.x}%`, top: `${node.y}%` }} data-testid={`node-${node.id}`}>
      ...
      <Button variant="outline" onClick={generatePuzzle} className="w-full" data-testid="button-reset">Reset Circuit</Button>
    </Card>
  );
}
