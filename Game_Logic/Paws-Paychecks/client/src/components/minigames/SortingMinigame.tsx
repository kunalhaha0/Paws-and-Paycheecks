...
type Ingredient = {
  id: string;
  name: string;
  category: "vegetable" | "protein" | "dairy" | "grain";
};
...
export function SortingMinigame({ onComplete }: SortingMinigameProps) {
  const [items, setItems] = useState<Ingredient[]>([]);
  const [sorted, setSorted] = useState<Record<string, Ingredient[]>>({
    vegetable: [],
    protein: [],
    dairy: [],
    grain: [],
  ...
  useEffect(() => {
    if (started && items.length === 0 && totalItems > 0) {
      setFinished(true);
    }
    ...
    return (
      <Card className="p-8 text-center space-y-6" data-testid="card-minigame-intro">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Sort the ingredients into the correct categories!</p>
        </div>
      ...
      <Card className="p-8 text-center space-y-6 pop-in" data-testid="card-minigame-result">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            {correctCount} items sorted correctly
          </p>
        </div>
      </Card>
    );
  ...
  return (
    <Card className="p-6 space-y-6" data-testid="card-minigame-active">
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground" data-testid="text-items-left">
      ...
      <div className="flex flex-wrap gap-3 justify-center min-h-[100px] p-4 bg-muted/50 rounded-lg">
        {items.map(item => (
          <button
            key={item.id}
            ...
            className={cn(
              ...
              "bg-card border shadow-sm hover-elevate",
              selected?.id === item.id && "ring-2 ring-primary scale-105"
            )}
            data-testid={`button-ingredient-${item.id}`}
          >
            <span className="font-medium">{item.name}</span>
          </button>
        ))}
        {items.length === 0 && (
          <div className="text-muted-foreground">All sorted!</div>
        )}
      </div>

      {selected && (
        <div className="text-center text-sm text-muted-foreground" data-testid="text-selected-item">
          Click a category to sort: <span className="font-medium">{selected.name}</span>
        </div>
      )}
          ...
          return (
            <button
              ...
              className={cn(
                ...
                selected ? "hover:border-primary hover:bg-primary/5 cursor-pointer" : "opacity-70 cursor-default",
                sorted[cat.id].length > 0 && "bg-muted/50"
              )}
            ...
            >
              ...
              <div className="text-sm text-muted-foreground">
                {sorted[cat.id].length > 0 && `${sorted[cat.id].length} items`}
              </div>
            </button>
          );
        })}
