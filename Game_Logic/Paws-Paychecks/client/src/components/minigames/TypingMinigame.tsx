...
import { Keyboard, CheckCircle, AlertCircle, Trophy } from "lucide-react";
...
const WORDS = [
  "report", "meeting", "email", "deadline", "project",
  "document", "schedule", "budget", "office", "client",
  "proposal", "contract", "invoice", "analysis", "review"
];

export function TypingMinigame({ onComplete }: TypingMinigameProps) {
    ...
    return (
      <Card className="p-8 text-center space-y-6" data-testid="card-minigame-intro">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-foreground">Typing Challenge</h2>
          <p className="text-muted-foreground">Type the words as fast as you can!</p>
        </div>
      ...
      <Card className="p-8 text-center space-y-6 pop-in" data-testid="card-minigame-result">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            {correctCount} / {words.length} words typed correctly
          </p>
        </div>
      </Card>
    );
  ...
  return (
    <Card className="p-8 space-y-6" data-testid="card-minigame-active">
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground" data-testid="text-word-progress">
      ...
      <div className="text-center space-y-4">
        <div className="text-4xl font-mono font-bold tracking-wider text-foreground" data-testid="text-current-word">
          {currentWord.split("").map((char, i) => (
            <span 
              key={i}
              className={cn(
                i < input.length && input[i] === char && "text-green-500",
                i < input.length && input[i] !== char && "text-destructive"
              )}
            >
              {char}
            </span>
        ...
        <input
          ...
          autoFocus
          className="w-full max-w-xs mx-auto px-4 py-3 text-xl text-center font-mono border-2 rounded-lg focus:outline-none focus:border-primary bg-background"
